...
document.querySelectorAll(".wind-toggle").forEach((toggle) => {
...
citySearchInput.addEventListener(
  "input",
  debounce((e) => searchCities(e, searchResults), 300),
)
citySearch1.addEventListener(
  "input",
  debounce((e) => searchCities(e, searchResults1), 300),
)
citySearch2.addEventListener(
  "input",
  debounce((e) => searchCities(e, searchResults2), 300),
)
...
async function searchCities(event, resultsElement) {
...
async function getCompareWeatherData(lat, lon, name, targetElement) {
  try {
    ...
    updateCompareUI(data, name, targetElement)
  } catch (error) {
    console.error("Error fetching comparison weather data:", error)
    targetElement.innerHTML = '<div class="error">Error fetching weather data. Please try again.</div>'
  }
}
...
function updateCompareUI(data, name, targetElement) {
...
function updateHourlyForecast(forecastData) {
  ...
  hourlyForecast.innerHTML = limitedForecasts
    .map((item) => {
...
function updateWeeklyForecast(forecastData) {
  ...
  weeklyForecast.innerHTML = limitedDays
    .map((date) => {
...
function updateComparisonTemperatures() {
  const city1Input = citySearch1.value
  const city2Input = citySearch2.value

  if (city1Input) {
    const recentSearches = JSON.parse(localStorage.getItem("recentSearches") || "[]")
    const cityData = recentSearches.find((city) => city.name === city1Input)
  ...
  if (city2Input) {
    const recentSearches = JSON.parse(localStorage.getItem("recentSearches") || "[]")
    const cityData = recentSearches.find((city) => city.name === city2Input)
...
function updateFavoriteButtonState(name) {
  ...
  if (isFavorite) {
    ...
    addFavoriteBtn.style.background = "#718096"
  } else {
    addFavoriteBtn.textContent = "Add Favorite"
    addFavoriteBtn.style.background = "#3182ce"
  }
}
    ...
    try {
      ...
      cityCard.innerHTML = `
        ...
        <div class="city-card-actions">
          <button class="view-city" title="View city">
      ...
      // Add event listeners
      const viewButton = cityCard.querySelector(".view-city")
      ...
      const removeButton = cityCard.querySelector(".remove-favorite")
...
// Ensure search inputs show results on focus
citySearchInput.addEventListener("focus", () => {
  if (citySearchInput.value.trim().length >= 2) {
    searchCities({ target: citySearchInput }, searchResults)
  }
})

citySearch1.addEventListener("focus", () => {
  if (citySearch1.value.trim().length >= 2) {
    searchCities({ target: citySearch1 }, searchResults1)
  }
})

citySearch2.addEventListener("focus", () => {
  if (citySearch2.value.trim().length >= 2) {
    searchCities({ target: citySearch2 }, searchResults2)
  }
})
